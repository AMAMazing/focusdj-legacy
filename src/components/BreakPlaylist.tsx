...
interface BreakPlaylistProps {
  onClose: (playlistName?: string) => void;
}

export const BreakPlaylist: React.FC<BreakPlaylistProps> = ({ onClose }) => {
  const { setBreakPlaylist } = useStore();
  const [loading, setLoading] = React.useState<string | null>(null);
  const [error, setError] = React.useState<string | null>(null);

  const handlePlaylistSelect = async (url: string, playlistId: string, name: string) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 animate-fade-in">
      <div className="bg-[#282828] rounded-xl p-6 w-full max-w-lg m-4 relative">
        ...
        <h2 className="text-xl font-bold text-white mb-4 text-center">Select a Break Playlist</h2>
        <div className="max-h-[60vh] overflow-y-auto custom-scrollbar p-1">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {newPlaylists.map((playlist) => (
              <button
                key={playlist.id}
                onClick={() => handlePlaylistSelect(playlist.url, playlist.id, playlist.name)}
                disabled={!!loading}
